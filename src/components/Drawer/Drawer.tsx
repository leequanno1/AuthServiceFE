...
const Drawer: React.FC = () => {
  ...
  const [accountPolicies, setAccountPolicies] =
    React.useState<AccountPolicies | null>();
  ...
  return (
    <nav className={`app-drawer ${isOpen ? "open" : "close"}`}>
      ...
      <div className="dw-menu-items">
        <Link className="nav head-nav" to={"/console-home"}>
          <h3>Console Home</h3>
        </Link>
        <Link className="nav head-nav" to={"/account-control"}>
          <h3>Account Control</h3>
        </Link>
        {(isRoot || !!accountPolicies?.canCreate) && (
          <Link className="nav sub-nav" to={"/account-control/create"}>
            Create sub-account
          </Link>
        )}
        <Link className="nav head-nav" to={"/pool-control"}>
          <h3>Pool Control</h3>
        </Link>
        {isRoot && (
          <Link className="nav sub-nav" to={"/pool-control/create"}>
            Create new pool
          </Link>
        )}
