...
const SubUserAndPoliciesInfo: React.FC = () => {
  ...
  const [policyList, setPolicList] = React.useState<Policy[]>([]);
  const [selectedPolicyList, setSelectedPolicList] = React.useState<Policy[]>(
    []
  );
  const [account, setAccount] = React.useState<Account | null>(null);
  const [lowerSubAccs, setLowerSubAccs] = React.useState<Account[]>([]);
  ...
  React.useEffect(() => {
    const initPoolPolicy = async () => {
      try {
        ...
        const tempLowerAccs = await accountService.getSubAccountsByParentID(
          accountId ?? ""
        );
