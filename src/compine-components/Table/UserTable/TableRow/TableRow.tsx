    ...
    const initSubAccountData = async () => {
      try {
        if (isExpanded && !isDataLoaded) {
          ...
          // Giả lập tải dữ liệu
          setSubAccounts(
            await accountService.getSubAccountsByParentID(
              account.accountId ?? ""
            )
          );
        }
        ...
        toastService.error(
          "An error occurred while loading sub-account's data."
        );
      }
    };
    initSubAccountData();
    ...
    <div className="row-wraper">
      ...
      >
        {!isSubRow ? (
          <div className="row-column-1">
            <input
              ...
              checked={isChecked}
              onChange={() => onCheckedChange(account.accountId ?? "")}
            />
          </div>
        ) : (
        ...
        <div className="row-column-2">
          <Link
            to={`/account-control/user/${account.accountId}`}
            rel="noopener noreferrer"
          >{`${account.displayName}/${account.username}`}</Link>
        </div>

        <div className="row-column-3">
          <span>{`by ${parentAcc?.username}--${
            account.createdAt ? DateService.formatDate(account.createdAt) : ""
          }`}</span>
